...
public class PreferenceBasic extends Preference {
    ...
    public interface Callback {
        void onPreferenceClicked(final ConnectedActivity activity, final Preference preference, final StoragePref storagePref, final OnPreferenceClickedCallback callback);
    ...
    @Nullable
    public static View getView(final ConnectedActivity activity, final PreferenceBasic preference, final StoragePref storagePref) {
        ...
        if (preference.summary != 0) {
            ...
            if (preference.changeSummary) {
                preference_basic_summary.setVisibility(View.VISIBLE);
                preference_basic_summary.setText(preference.callback.onGetSummary(activity, storagePref.get(activity, preference.key, preference.defaultValue == null ? "" : (String) preference.defaultValue)));
            } else {
        ...
        preference_basic.setOnClickListener(view -> {
            ...
            preference.callback.onPreferenceClicked(activity, preference, storagePref, (a, v) -> Thread.runOnUI(() -> {
