...
public class ERegisterSubjectViewRVA extends RVA {
    ...
    private static Pattern patternHighlight = Pattern.compile("^.*зач[её]т$|^экзамен$|^модуль\\s\\d+$|^промежуточная\\sаттестация$|^защита\\s(кп/кр|кп|кр|курсового\\sпроекта|курсовой\\sработы|курсового\\sпроекта/курсовой\\sработы)$|^тесты$", Pattern.CASE_INSENSITIVE);
